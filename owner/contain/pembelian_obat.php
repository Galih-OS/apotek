...
if(isset($_POST['tambah_pemesanan']))
{
	...
	$rs = mysql_query("SELECT COUNT(*) FROM pemesanan WHERE tanggal LIKE '%".$pencarian."%' ") or die("Count query error!");
...
<?php
		...
		define('MAX_REC_PER_PAGE', 5);
		$rs = mysql_query("SELECT COUNT(*) FROM pemesanan") or die("Count query error!");
		list($total) = mysql_fetch_row($rs);
		$total_pages = ceil($total / MAX_REC_PER_PAGE);
		$page = intval(@$_GET["page"]);
