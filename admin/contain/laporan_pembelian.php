...
<div class="panel panel-default">
  <div class="panel-body">
	<ul class="nav nav-tabs">
	  <li role="presentation"><a href="index.php?contain=laporan_pemesanan">Permintaan</a></li>
	  <li role="presentation" class="active"><a href="index.php?contain=laporan_pembelian">Pembelian</a></li>
...
if(isset($_POST['edit']))
{
	...
	include "atribut/detail_pembelian.php";
...
?>
	...
	<form class="form-horizontal" action="contain/atribut/cetak_pembelian.php" method="POST">
...
<?php	
	...
	$rs = mysql_query("SELECT COUNT(*) FROM pembelian
						WHERE MONTH(pembelian.tanggal_diterima) = substr('".$bulan."', -1, 2) ") or die("Count query error!");
	...
	$rs = mysql_query("SELECT * FROM pembelian
							JOIN pengguna ON pembelian.id_pengguna = pengguna.id_pengguna
							JOIN supplier ON pembelian.id_supplier = supplier.id_supplier
							WHERE MONTH(pembelian.tanggal_diterima) = substr('".$bulan."', -1, 2)
							ORDER BY pembelian.tanggal_diterima ASC
...
?>	
	<table border="1" width="95%" align="center" class="table">
		<thead>
			...
			<td class="success" style="text-align:center;">TANGGAL DITERIMA</td>
		...
		<tr>
			...
			<td align="center">
				<?php echo $list['id_pembelian']; ?>
				<input hidden name='id_pembelian' value="<?php echo $list['id_pembelian']?>">
			</td>
			<td align="center">
		...
		<tr>
			...
			<?php
				for ($i = 1; $i <= $total_pages; $i++) {
					$txt=$i;
					if ($page != $i)
						$txt = "<a href=\"" . $_SERVER["PHP_SELF"] . "?contain=laporan_pembelian&page=$i\">$txt</a>";
			?>
				<strong><?= $txt ?></strong>
...
?>	
	...
	<div class="panel panel-default">
	  <div class="panel-heading">
		<h3 class="panel-title" align="center">Laporan Pembelian Bulan <?php echo date('F Y'); ?></h3>
	  </div>
	  <div class="panel-body">
	...
	<?php
		...
		define('MAX_REC_PER_PAGE', 10);
		$rs = mysql_query("SELECT COUNT(id_pembelian) FROM pembelian
							JOIN pengguna ON pembelian.id_pengguna = pengguna.id_pengguna
							JOIN supplier ON pembelian.id_supplier = supplier.id_supplier
							WHERE MONTH(pembelian.tanggal_diterima) = DATE_FORMAT(NOW(),'%m')
							ORDER BY pembelian.tanggal_diterima ASC") or die("MAINTENANCE!");
		list($total) = mysql_fetch_row($rs);
		$total_pages = ceil($total / MAX_REC_PER_PAGE);
		$page = intval(@$_GET["page"]);
