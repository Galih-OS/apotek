...
<div class="panel panel-default">
  <div class="panel-body">
	<ul class="nav nav-tabs">
	  ...
	  <li role="presentation" class="active"><a href="index.php?contain=laporan_penjualan">Penjualan</a></li>
	</ul>
...
if(isset($_POST['edit']))
{
	...
	include "atribut/detail_penjualan.php";
...
?>
	...
	<form class="form-horizontal" action="contain/atribut/cetak_penjualan.php" method="POST">
...
<?php	
	...
	$rs = mysql_query("SELECT COUNT(id_penjualan) FROM penjualan
						JOIN pengguna ON penjualan.id_pengguna = pengguna.id_pengguna
						WHERE MONTH(penjualan.tanggal_penjualan) = substr('".$bulan."', -1, 2) ") or die("Count query error!");
	...
	$rs = mysql_query("SELECT * FROM penjualan
							JOIN pengguna ON penjualan.id_pengguna = pengguna.id_pengguna
							JOIN detail_penjualan ON penjualan.id_penjualan = detail_penjualan.id_penjualan
							WHERE MONTH(penjualan.tanggal_penjualan) = substr('".$bulan."', -1, 2)
	...
	<table border="1" width="95%" align="center" class="table">
		...
		<tr>
			...
			<td align="center">
				...
				<input hidden name='id_penjualan' value="<?php echo $list['id_penjualan']?>">
			</td>
			<td align="center">
				<?php echo $list['tanggal_penjualan']; ?>
				<input hidden name='tanggal' value="<?php echo $list['tanggal_penjualan']?>">
			</td>
			<td align="center">
			...
			<td align="center">
				...
				<input hidden name='total_penjualan' value="<?php echo $list['total_penjualan']?>">
			</td>
			<td align="center">
		...
		<tr>
			...
			<?php
				for ($i = 1; $i <= $total_pages; $i++) {
					$txt=$i;
					if ($page != $i)
						$txt = "<a href=\"" . $_SERVER["PHP_SELF"] . "?contain=laporan_penjualan&page=$i\">$txt</a>";
			?>
				<strong><?= $txt ?></strong>
...
?>	
	...
	<?php
		...
		$rs = mysql_query("SELECT COUNT(id_penjualan) FROM penjualan
							JOIN pengguna ON penjualan.id_pengguna = pengguna.id_pengguna
							WHERE MONTH(penjualan.tanggal_penjualan) = DATE_FORMAT(NOW(),'%m')
							ORDER BY penjualan.tanggal_penjualan ASC") or die("MAINTENANCE!");
		list($total) = mysql_fetch_row($rs);
		$total_pages = ceil($total / MAX_REC_PER_PAGE);
		$page = intval(@$_GET["page"]);
