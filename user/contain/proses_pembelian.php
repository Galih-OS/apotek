...
?>
	...
	<?php
		$query_data_2 = mysql_query("SELECT COUNT(*) AS total_data_2 FROM detail_pembelian
							WHERE id_pembelian = '".$_SESSION['id_pemesanan']."' ") or die("Query error count!");
	...
	if(isset($_POST['check']))
	{
		...
		//UPDATE TOTAL HARGA PEMBELIAN
			//AMBIL NILAI DATA TERAKHIR
				$query_used = mysql_query("SELECT jumlah AS total_jumlah FROM pembelian
										WHERE id_pembelian = '".$id_pemesanan."' ") or die("Query used eror!");
						$total_jumlah = mysql_fetch_array($query_used);
						$total_jumlah = $total_jumlah['total_jumlah'];
			//MENAMBAH JUMLAH LAMA DAN BARU
				$total_jumlah += $harga;
			//MERUBAH NILAI DATA
			$update_pembelian = mysql_query("UPDATE pembelian
											SET jumlah = '".$total_jumlah."'
											WHERE id_pembelian='".$_SESSION['id_pemesanan']."' ");
		...
		// MENCARI STOK OBAT
		$query_stok_obat = mysql_query("SELECT jumlah_stok AS stok_obat FROM stok
		...
		$ubah_stok = mysql_query("UPDATE stok
								SET jumlah_stok = '".$total_stok_obat."'
		...
		// UPDATE NILAI HARGA BELI
			$update_harga_beli = mysql_query("UPDATE obat
